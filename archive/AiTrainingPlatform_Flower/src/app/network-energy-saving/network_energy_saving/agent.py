...
def train_fn(actor, critic, train_dataset, valid_dataset, batch_size=32, epochs=10, lr=1e-4, gamma=0, tau=0.1, device='cpu'):
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0 or epoch == epochs - 1:
            torch.save(actor.state_dict(), f'./network_energy_saving/models/test_actor_epoch_{epoch}.pt')
            torch.save(critic.state_dict(), f'./network_energy_saving/models/test_critic_epoch_{epoch}.pt')
            ...
            plt.tight_layout()
            plt.savefig(f'./network_energy_saving/results/training_loss.png')
            plt.close()

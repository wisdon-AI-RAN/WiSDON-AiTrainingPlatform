...
class DataLoader:
    ...
    def __init__(self, 
        ...
        self.collection_name = f"{self.project_id}_{self.app_name}_{self.model_name}_{self.model_version}_{self.mode}_{self.dataset_name}"
    ...
    def create_dataset(self, episode, split=0.1, random_seed=42):
        ...
        states, actions, rewards, next_states = self.connect_mongodb(episode) # load data from MongoDB
    ...
    def connect_mongodb(self, episode):
        try:
            # Connect to MongoDB
            ...
            states, actions, rewards, next_states = self.data_to_numpy(item_set, num_items)

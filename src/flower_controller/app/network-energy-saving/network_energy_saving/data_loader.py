...
class DataLoader:
    ...
    def create_dataset(self, episode, split=0.1):
        #===== Preprocess data =====#
        # print("Start data preprocessing...")
        # states, actions, rewards, next_states, dones = self.load_data() # load data from local npz file
        states, actions, rewards, next_states, dones = self.connect_mongodb(episode) # load data from MongoDB
    ...
    def connect_mongodb(self, episode):
        try:
            ...
            client = MongoClient(f'mongodb://mongodb:{self.port}', serverSelectionTimeoutMS=5000)
...
#         model_version="0.0.1"
#     )
#     states, actions, rewards, next_states, dones = data_loader.connect_mongodb()

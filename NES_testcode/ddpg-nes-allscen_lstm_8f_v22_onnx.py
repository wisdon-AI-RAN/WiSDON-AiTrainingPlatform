...
def export_actor_onnx(agent: DDPGAgent, onnx_path: str, opset: int = 17):
    """
    ...
    wrapper = ActorLogitsWrapper(agent.actor).eval()

    dummy = torch.zeros(1, N_FEATS, 10, TOTAL_BS, dtype=torch.float32, device="cuda")
    torch.onnx.export(
        wrapper,
        dummy,
        onnx_path,
...
if __name__ == "__main__":
    ...
    try:
        load_ddpg(agent, best_ckpt)
        onnx_path = best_ckpt.replace(".pth", "_actor_logits.onnx")
        export_actor_onnx(agent, onnx_path)
